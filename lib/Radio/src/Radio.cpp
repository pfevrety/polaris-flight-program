...
bool RADIO::begin(int csPin, int resetPin, int irqPin) {
    ...
    // Test SPI manuel — lit le registre version du RFM95W
    // Doit retourner 0x12 si le module répond
    pinMode(csPin, OUTPUT);
    digitalWrite(csPin, LOW);
    SPI.begin();
    SPI.beginTransaction(SPISettings(1000000, MSBFIRST, SPI_MODE0));
    digitalWrite(csPin, LOW);
    SPI.transfer(0x42); // registre RegVersion
...
void RADIO::sendFlightData(const FlightData &fd) {
    if (!lastTransmissionFinished) return;

    LoRa.beginPacket();
    LoRa.write((uint8_t*)&fd, sizeof(FlightData));
    LoRa.endPacket(true); // true = mode async (non bloquant)
